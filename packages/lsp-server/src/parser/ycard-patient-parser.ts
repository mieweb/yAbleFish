...
const AddressSchema = z.object({
  ...
  postal_code: z.union([z.string(), z.number()]).transform(val => String(val)).optional(),
...
export const MedicalPatientSchema = z.object({
  ...
  dob: z.union([z.string(), z.date()]).transform(val => val instanceof Date ? val.toISOString().split('T')[0] : val).optional(), // Date of Birth (ISO format)
  dateOfBirth: z.union([z.string(), z.date()]).transform(val => val instanceof Date ? val.toISOString().split('T')[0] : val).optional(), // Alternative format
  ...
  lastVisit: z.union([z.string(), z.date()]).transform(val => val instanceof Date ? val.toISOString().split('T')[0] : val).optional(), // ISO date
  nextAppointment: z.union([z.string(), z.date()]).transform(val => val instanceof Date ? val.toISOString().split('T')[0] : val).optional(), // ISO date
...
export class PatientParser {
   ...
   */
  parsePatientSection(content: string): { success: true; data: MedicalPatient } | { success: false; errors: string[] } {
    try {
      ...
      if (!patientMatch) {
        return { success: false, errors: ['No Patient section found in document'] };
      }
  ...
  validatePatient(patient: MedicalPatient): string[] {
    ...
    if (!patient.name && !patient.surname) issues.push('Patient name is required');
    ...
    if (patient.email && typeof patient.email === 'string' && !patient.email.includes('@')) {
